...
The last two columns are a different kind of claim from the rest. `ffmpeg` runs whatever
FFmpeg is installed and `mf` asks Media Foundation, whose audio codecs differ between a
desktop and a server Windows — so those cells are what *this* machine does, and
another machine measuring them differently is information rather than a regression.
Everything to their left is code in this repository, and is held to the table everywhere.
