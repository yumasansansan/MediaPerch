...
| M4 | Path B: f64 bus, DSP chain, resampler, dither. Gapless, seek | **the bus, the chain, the resampler and the dither are done**, and the passthrough path still contains no float. Gapless, seek, and switching paths at a rebuild point without a glitch are not |
