...
conversion: the bus is built only when there is a chain to put on it. Nothing that
resamples or remixes is written yet, so negotiation still refuses a device that wants a
rate the file does not have -- but that is now a missing module rather than a missing
mechanism, which is what M4 was for.
...
| M4 | Path B: f64 bus, DSP chain, resampler, dither. Gapless, seek | switching paths at a rebuild point is glitch-free and the passthrough path still contains no float |
