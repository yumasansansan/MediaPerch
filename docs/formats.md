...
| MP3, 44.1 kHz stereo 256k | `demux_mpa` + `codec_mpa` | `69dca145…769ffbf33` | **changed, on purpose** |
...
**The MP3 row is the one exception and it is not the split's doing.** That cell
read `c86e36b4…` while `codec_mpa` was dr_mp3; it is libmpg123 now, and a lossy
codec with no normative bit-exact decoder has no obligation to agree with the one
it replaced. The two agree with FFmpeg to 113 dB, which is the measurement that
means something. **It is also the one row that depends on how the tree was
built** -- libmpg123 has an AVX synthesis and a generic one, they differ by 128
dB of float rounding, and which you get depends on whether `yasm` was on PATH.
This tree takes the AVX one and says so out loud when it cannot; see
`modules/codec/mpa/CMakeLists.txt` for the whole of that story. The three
