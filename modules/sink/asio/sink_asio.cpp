...
// The registry half of driver discovery lives in asio_registry.cpp, on the
// other side of the MEDIAPERCH_EXTERNAL line -- see that file for why.
using mp::asio::Registered;
using mp::asio::registered_drivers;
...
try {
    ...
    *out = nullptr;
    ...
    // Parentheses rather than braces: value-initialisation either way for
    // this struct, but the analyzer reports `new (std::nothrow) T{}` as a
    // null dereference and `T()` as what it is.
